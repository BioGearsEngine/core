...
public:
  MyListener(Logger* logger)
    : SEEventHandler()
    , log(logger) {};
  virtual void HandlePatientEvent(SEPatientEventType type, bool active, const SEScalarTime* time) override
  {
    std::stringstream ss;
  ...
  virtual void HandleAnesthesiaMachineEvent(SEAnesthesiaMachineEvent type, bool active, const SEScalarTime* time) override
  {
    std::stringstream ss;
