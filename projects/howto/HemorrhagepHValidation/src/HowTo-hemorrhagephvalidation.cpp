...
#include "HowTo-hemorrhagephvalidation.h"
#include <iostream>
  ...
  : m_hemThread()
{
  ...
  m_hemorrhage->GetInitialRate().SetValue(hemFlow, VolumePerTimeUnit::mL_Per_min);
...
void HemThread::SetRingersInfusionRate(double& volume, double& rate)
{
...
void HemThread::SetBloodInfusionRate(double& volume, double& rate)
{
...
void HemThread::SetPlasmaInfusionRate(double& volume, double& rate)
{
...
void HemThread::SetPlateletInfusionRate(double& volume, double& rate)
{
...
void HemThread::AdvanceTimeFluids()
{
  m_mutex.lock();
...
void HemThread::FluidLoading(double hemFlow)
{
  ...
  SEArterialBloodGasAnalysis bga;
