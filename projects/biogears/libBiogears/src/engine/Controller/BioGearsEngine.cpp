...
namespace biogears {
//-------------------------------------------------------------------------------
BioGearsEngine::BioGearsEngine(std::string const& logFileName)
  : BioGearsEngine(logFileName, Loggable::empty)
{
}
//-------------------------------------------------------------------------------
BioGearsEngine::BioGearsEngine(const char* logFileName)
  : BioGearsEngine(std::string { logFileName }, Loggable::empty)
{
}
//-------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetLogger() const -> Logger const*
{
  return m_Logger;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetLogger() -> Logger*
{
  // TODO::Refactor to allow BioGearsEngine to keep a mutableLogger
  //       Maybe keep a pointer to an m_unmanagedLogger
  if (m_managedLogger) {
    return m_managedLogger.get();
  }
  return const_cast<Logger*>(m_Logger);
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetEngineTrack() const -> PhysiologyEngineTrack const&
{
  return m_EngineTrack;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetEngineTrack() -> PhysiologyEngineTrack&
{
  return m_EngineTrack;
}
//-------------------------------------------------------------------------------
...
{
  auto requests = m_EngineTrack.GetDataRequestManager().GetDataRequests();
  auto resultsFile = m_EngineTrack.GetDataRequestManager().GetResultsFilename();
  ...
  m_SimulationPhase = SimulationPhase::Active;
...
//-------------------------------------------------------------------------------
std::unique_ptr<CDM::PhysiologyEngineStateData> BioGearsEngine::GetStateData() const
{
...
//-------------------------------------------------------------------------------
auto BioGearsEngine::SaveStateToFile(const char* file) const -> void
{
  SaveStateToFile(std::string { file });
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::SaveStateToFile(const std::string& file) const -> void
{
...
bool BioGearsEngine::InitializeEngine(const std::vector<const SECondition*>* conditions, const PhysiologyEngineConfiguration* config)
{
  ...
  m_SimulationPhase = SimulationPhase::Initialization;
  ...
  m_SimulationPhase = SimulationPhase::InitialStabilization;
  if (!m_Config->GetStabilizationCriteria()->StabilizeRestingState(*this)) {
    return false;
  }
  // We need to process conditions here, so systems can prepare for them in their SimulationPhaseChange method
  ...
  SimulationPhaseChange(SimulationPhase::AtInitialStableState);

  m_SimulationPhase = SimulationPhase::SecondaryStabilization;
  ...
  SimulationPhaseChange(SimulationPhase::AtSecondaryStableState);
  ...
  //! Run this again to clear out any bumps from systems resetting baselines in the last SimulationPhaseChange call
...
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetTimeStep(const TimeUnit& unit) const -> double
{
  return m_Config->GetTimeStep(unit);
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetTimeStep() -> SEScalarTime&
{
  return m_Config->GetTimeStep();
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetTimeStep() const -> SEScalarTime const&
{
  return m_Config->GetTimeStep();
}

//-------------------------------------------------------------------------------
auto BioGearsEngine::GetSimulationTime(const TimeUnit& unit) const -> double
{
  return m_SimulationTime->GetValue(unit);
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetSimulationTime() -> SEScalarTime&
{
  return *m_SimulationTime;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetSimulationTime() const -> SEScalarTime const&
{
  return *m_SimulationTime;
}
//-------------------------------------------------------------------------------
bool BioGearsEngine::AdvanceModelTime(bool appendDataTrack)
{
  ...
  const auto sample_interval_s = 1.0 / m_EngineTrack.GetDataRequestManager().GetSamplesPerSecond();
  ...
  if (m_timeSinceLastDataTrack >= sample_interval_s) {
    ...
    if ((m_isAutoTracking && SimulationPhase::Active == m_SimulationPhase) || (m_SimulationPhase < SimulationPhase::Active && m_areTrackingStabilization)) {
      auto time = GetSimulationTime(TimeUnit::s);
      m_EngineTrack.TrackData(time, appendDataTrack);
    }
  }
...
bool BioGearsEngine::ProcessAction(const SEAction& action)
{
  ...
  if (assessment != nullptr) {
    m_ss.str("");
    std::string results_filepath = m_EngineTrack.GetDataRequestManager().GetResultsFilename();
...
//-------------------------------------------------------------------------------
bool BioGearsEngine::IsReady() const
{
  if (m_SimulationPhase == SimulationPhase::NotReady) {
...
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetConfiguration() const -> BioGearsConfiguration const&
{
  return *m_Config;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetConfiguration() -> BioGearsConfiguration&
{
  return *m_Config;
}

//-------------------------------------------------------------------------------
auto BioGearsEngine::GetSaturationCalculator() -> SaturationCalculator&
{
  return *m_SaturationCalculator;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetSaturationCalculator() const -> SaturationCalculator const&
{
  return *m_SaturationCalculator;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetDiffusionCalculator() -> DiffusionCalculator&
{
  return *m_DiffusionCalculator;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetDiffusionCalculator() const -> DiffusionCalculator const&
{
  return *m_DiffusionCalculator;
}
//-------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetActions() const -> SEActionManager const&
{
  return *m_Actions;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetPatient() const -> SEPatient const&
{
  return *m_Patient;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetPatient() -> SEPatient&
{
  return *m_Patient;
}
//-------------------------------------------------------------------------------
bool BioGearsEngine::GetPatientAssessment(SEPatientAssessment& assessment) const
{
  if (!IsReady()) {
    return false;
  }
...
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetEngineTime() const -> SEScalarTime const&
{
  return *m_CurrentTime;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetEngineTime() -> SEScalarTime const&
{
  return *m_CurrentTime;
}
//-------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetAirwayMode() const -> CDM::enumBioGearsAirwayMode::value const
{
  return m_AirwayMode;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetIntubation() const -> CDM::enumOnOff::value const
{
  return m_Intubation;
}
//-------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetCircuits() const -> BioGearsCircuits const&
{
  return *m_Circuits;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetCircuits() -> BioGearsCircuits&
{
  return *m_Circuits;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetCompartments() const -> BioGearsCompartments const&
{
  return *m_Compartments;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetCompartments() -> BioGearsCompartments&
{
  return *m_Compartments;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetConditions() const -> SEConditionManager const&
{
  return *m_Conditions;
}
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetConditions() -> SEConditionManager&
{
  return *m_Conditions;
}
//-------------------------------------------------------------------------------
Tree<const char*> BioGearsEngine::GetDataRequestGraph() const
{
  return Tree<const char*> { "BioGearsEngine" }
    .emplace_back(GetBloodChemistrySystem().GetPhysiologyRequestGraph())
    .emplace_back(GetCardiovascularSystem().GetPhysiologyRequestGraph())
    .emplace_back(GetDrugsSystem().GetPhysiologyRequestGraph())
    .emplace_back(GetEndocrineSystem().GetPhysiologyRequestGraph())
    .emplace_back(GetEnergySystem().GetPhysiologyRequestGraph())
    .emplace_back(GetGastrointestinalSystem().GetPhysiologyRequestGraph())
    .emplace_back(GetHepaticSystem().GetPhysiologyRequestGraph())
    .emplace_back(GetNervousSystem().GetPhysiologyRequestGraph())
    .emplace_back(GetRenalSystem().GetPhysiologyRequestGraph())
    .emplace_back(GetRespiratorySystem().GetPhysiologyRequestGraph())
    .emplace_back(GetTissueSystem().GetPhysiologyRequestGraph());
}
...
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetSimulationPhase() const -> SimulationPhase
{
  return m_SimulationPhase;
}
//-------------------------------------------------------------------------------
...
auto BioGearsEngine::GetDataTrack() -> DataTrack&
{
...
//-------------------------------------------------------------------------------
auto BioGearsEngine::GetDataTrack() const -> DataTrack const&
{
  return const_cast<BioGearsEngine*>(this)->GetDataTrack();
}

//---------------------------------------------------------------------
void BioGearsEngine::SetUp()
{
  m_DataTrack = nullptr;
...
bool BioGearsEngine::Initialize(const PhysiologyEngineConfiguration* config)
{
  ...
  // TODO Refactor for BioGearsEngine to retain a mutable pointer to Logger
  const_cast<Logger*>(m_Logger)->SetLogTime(m_SimulationTime.get());
...
//---------------------------------------------------------------------
void BioGearsEngine::SetAirwayMode(CDM::enumBioGearsAirwayMode::value mode)
{
  ...
  Info(asprintf("Airway Mode : %s",
                (CDM::enumBioGearsAirwayMode::Free == m_AirwayMode) ? "Free" : (CDM::enumBioGearsAirwayMode::AnesthesiaMachine == m_AirwayMode) ? "AnesthesiaMachine"
                  : (CDM::enumBioGearsAirwayMode::Inhaler == m_AirwayMode)                                                                      ? "Inhaler"
                  : (CDM::enumBioGearsAirwayMode::MechanicalVentilator == m_AirwayMode)                                                         ? "MechanicalVentilator"
                                                                                                                                                : "Unknown"));
}
//---------------------------------------------------------------------
void BioGearsEngine::SetIntubation(CDM::enumOnOff::value s)
{
  if (m_Intubation == s) {
    return; // do nazing!
  }
...
//---------------------------------------------------------------------
bool BioGearsEngine::SetupPatient()
{
  bool err = false;
...
//---------------------------------------------------------------------
void BioGearsEngine::SimulationPhaseChange(SimulationPhase state)
{
  m_SimulationPhase = state;
  m_Environment->SimulationPhaseChange();
  m_CardiovascularSystem->SimulationPhaseChange();
  m_Inhaler->SimulationPhaseChange();
  m_RespiratorySystem->SimulationPhaseChange();
  m_AnesthesiaMachine->SimulationPhaseChange();
  m_GastrointestinalSystem->SimulationPhaseChange();
  m_HepaticSystem->SimulationPhaseChange();
  m_RenalSystem->SimulationPhaseChange();
  m_EnergySystem->SimulationPhaseChange();
  m_EndocrineSystem->SimulationPhaseChange();
  m_DrugSystem->SimulationPhaseChange();
...
//---------------------------------------------------------------------
void BioGearsEngine::SetupCardiovascular()
{
...
//---------------------------------------------------------------------
void BioGearsEngine::SetupCerebral()
{
...
//---------------------------------------------------------------------
void BioGearsEngine::SetupRenal()
{
...
//---------------------------------------------------------------------
void BioGearsEngine::SetupTissue()
{
...
//---------------------------------------------------------------------
void BioGearsEngine::SetupRespiratory()
{
...
//---------------------------------------------------------------------
void BioGearsEngine::SetupGastrointestinal()
{
...
//---------------------------------------------------------------------
void BioGearsEngine::SetupAnesthesiaMachine()
{
...
//---------------------------------------------------------------------
void BioGearsEngine::SetupInhaler()
{
...
//---------------------------------------------------------------------
void BioGearsEngine::SetupMechanicalVentilator()
{
...
//---------------------------------------------------------------------
void BioGearsEngine::SetupExternalTemperature()
{
...
//---------------------------------------------------------------------
void BioGearsEngine::SetupInternalTemperature()
{
