...
void Drugs::CalculateDrugEffects()
{
  ...
  for (SESubstance* sub : m_data.GetCompartments().GetLiquidCompartmentSubstances()) {
    ...
    hemorrhageFlowRecoveryFraction += ((pd.GetHemorrhageModifier().GetValue()*10e-5) * concentrationEffects_unitless); // If the substance affects hemorrhage blood flow, scale unitless modifier dowwn to account for resistance sensitivity
