...
void Drugs::AdministerSubstanceBolus()
{
  ...
  const auto& boluses = m_data.GetActions().GetPatientActions().GetSubstanceBoluses();
  if (boluses.empty())
    return;
...
void Drugs::AdministerSubstanceInfusion()
{
  ...
  const auto& infusions = m_data.GetActions().GetPatientActions().GetSubstanceInfusions();
  if (infusions.empty())
    return;
...
{
  const auto& nasalDoses = m_data.GetActions().GetPatientActions().GetSubstanceNasalDoses();
  if (nasalDoses.empty())
    return;
...
void Drugs::AdministerSubstanceOral()
{
  ...
  const auto& oralDoses = m_data.GetActions().GetPatientActions().GetSubstanceOralDoses();
  if (oralDoses.empty())
    return;
...
{
  const auto& infusions = m_data.GetActions().GetPatientActions().GetSubstanceCompoundInfusions();
  if (infusions.empty())
    return;
