...
namespace io {
  ...
  void Circuit::UnMarshall(const CDM::ElectricalCircuitData& in, SEElectricalCircuit& out)
  {
    //Nothing to Do
  }
  //----------------------------------------------------------------------------------
  ...
  void Circuit::UnMarshall(const CDM::ElectricalCircuitNodeData& in, SEElectricalCircuitNode& out)
  {
  ...
  void Circuit::UnMarshall(const CDM::ElectricalCircuitPathData& in, SEElectricalCircuitPath& out)
  {
  ...
  void Circuit::UnMarshall(const CDM::FluidCircuitData& in, SEFluidCircuit& out)
  {
    //Nothing to Do
  }
  //----------------------------------------------------------------------------------
  ...
  void Circuit::UnMarshall(const CDM::FluidCircuitNodeData& in, SEFluidCircuitNode& out)
  {
  ...
  void Circuit::UnMarshall(const CDM::FluidCircuitPathData& in, SEFluidCircuitPath& out)
  {
  ...
  void Circuit::UnMarshall(const CDM::ThermalCircuitData& in, SEThermalCircuit& out)
  {
    //Nothing to do
  }
  //----------------------------------------------------------------------------------
  ...
  void Circuit::UnMarshall(const CDM::ThermalCircuitNodeData& in, SEThermalCircuitNode& out)
  {
  ...
  void Circuit::UnMarshall(const CDM::ThermalCircuitPathData& in, SEThermalCircuitPath& out)
  {
  ...
  void Circuit::UnMarshall(const CDM::CircuitManagerData& in, SECircuitManager& out)
  {
    ...
    for (const CDM::ElectricalCircuitNodeData& n : in.ElectricalNode()) {
      io::Circuit::UnMarshall(n, out.CreateNode<ELECTRICAL_LEDGER_TYPES>(n.Name(), out.m_ElectricalLedger));
    }
    for (const CDM::ElectricalCircuitPathData& p : in.ElectricalPath()) {
      ...
      io::Circuit::UnMarshall(p, out.CreatePath<ELECTRICAL_LEDGER_TYPES>(*src, *tgt, p.Name(), out.m_ElectricalLedger));
    }
    for (const CDM::ElectricalCircuitData& c : in.ElectricalCircuit()) {
      io::Circuit::UnMarshall(c,out.CreateCircuit<ELECTRICAL_LEDGER_TYPES>(c.Name(), out.m_ElectricalLedger), out.m_ElectricalLedger.nodes, out.m_ElectricalLedger.paths);
    }
    for (const CDM::FluidCircuitNodeData& n : in.FluidNode()) {
      io::Circuit::UnMarshall(n, out.CreateNode<FLUID_LEDGER_TYPES>(n.Name(), out.m_FluidLedger));
    }
    for (const CDM::FluidCircuitPathData& p : in.FluidPath()) {
      ...
      io::Circuit::UnMarshall(p, out.CreatePath<FLUID_LEDGER_TYPES>(*src, *tgt, p.Name(), out.m_FluidLedger));
    }
    for (const CDM::FluidCircuitData& c : in.FluidCircuit())
      io::Circuit::UnMarshall(c, out.CreateCircuit<FLUID_LEDGER_TYPES>(c.Name(), out.m_FluidLedger));

    for (const CDM::ThermalCircuitNodeData& n : in.ThermalNode())
      io::Circuit::UnMarshall(n, out.CreateNode<THERMAL_LEDGER_TYPES>(n.Name(), out.m_ThermalLedger));
    for (const CDM::ThermalCircuitPathData& p : in.ThermalPath()) {
      ...
      io::Circuit::UnMarshall(p, out.CreatePath<THERMAL_LEDGER_TYPES>(*src, *tgt, p.Name(), out.m_ThermalLedger));
    }
    for (const CDM::ThermalCircuitData& c : in.ThermalCircuit())
      io::Circuit::UnMarshall(c, out.CreateCircuit<THERMAL_LEDGER_TYPES>(c.Name(), out.m_ThermalLedger));
  }
  //----------------------------------------------------------------------------------
