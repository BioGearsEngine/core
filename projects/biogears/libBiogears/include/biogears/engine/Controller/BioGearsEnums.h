...
enum class SEBioGearsAirwayMode {
  Invalid = -1,
            Free = 0,
            AnesthesiaMachine,
            Inhaler,
...
inline std::string ToString(const SEBioGearsAirwayMode& bit)
{
  switch (bit) {
...
inline std::ostream& operator<<(std::ostream& os, const SEBioGearsAirwayMode& bit)
{
  os << ToString(bit);
  return os;
}
