...
{
  enum testCases { ExtraIntra = 0, VascExtra = 1, Combined = 2 };
  ...
  case ExtraIntra:
    ...
    case VascExtra:
      ...
      for (int j = 0; j < simTime * 60 / dt; j++)
      {
        ...
        massMoved_ug = pEx1ToEx2.GetFlow(VolumePerTimeUnit::mL_Per_s)*(1 - albReflection)*((cMuscle.GetSubstanceQuantity(Alb)->GetConcentration(MassPerVolumeUnit::ug_Per_mL)
          -exp(-pecletNum)*cExtraCell.GetSubstanceQuantity(Alb)->GetConcentration(MassPerVolumeUnit::ug_Per_mL)) / (1 - exp(-pecletNum)))*dt;
