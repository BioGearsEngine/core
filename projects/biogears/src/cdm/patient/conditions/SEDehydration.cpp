...
SEDehydration::SEDehydration() : SEPatientCondition()
{
...
void SEDehydration::ToString(std::ostream &str) const
{
	...
	if(HasComment())
		str<<"\n\tComment: "<<m_Comment;
	str << "\n\tFractional amount of body weight change due to fluid loss: "; HasDehydrationFraction() ? str << m_DehydrationFraction : str << "NaN";
	str << std::flush;
}