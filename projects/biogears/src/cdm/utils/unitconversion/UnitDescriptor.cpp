...
CUnitDescriptor::CUnitDescriptor(const std::string &name, const std::string &symbol, unsigned int quantityTypeId, 
								 ...
								 : m_strName(name), m_strSymbol(symbol), m_uiQuantityTypeId(quantityTypeId),
								 m_uiPrefixMode(mode), m_strPrefixSet(prefixSet)
{
	...
	if (targetCompoundUnit != "*" && targetCompoundUnit != "")
	{
		...
		if (*qtDim != *tgtDim)
		{
			std::cerr << "Target unit " << targetCompoundUnit << 
				" is dimensionaly incompatible with quantity type " <<
				qtDesc.GetName() << std::endl;
		}
