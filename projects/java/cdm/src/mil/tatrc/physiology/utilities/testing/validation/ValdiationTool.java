...
public abstract class ValdiationTool {
  ...
  protected enum DataType { Mean,
    ...
    Patient2SystemMax,
    MeanPerWeight,
    WaveformMinPerWeight,
    WaveformMaxPerWeight }
            ...
            for (int r = 0; r < rows; r++) {
              ...
              for (int c = 0; c <= cells; c++) {
                ...
                case 0: //A
                  ...
                  break;
                case 1: //B
                  if (cellValue != null && !cellValue.equalsIgnoreCase("none") && !cellValue.equalsIgnoreCase("n\\a") && !cellValue.equalsIgnoreCase("n/a")) {
                    vRow.unit = cellValue;
                  }
  ...
  protected boolean buildExpectedHeader(ValidationRow vRow)
  {
    ...
    if (rVals.length == 1) {
      ...
      if (vRow.dType == DataType.Mean || vRow.dType == DataType.MeanPerWeight || vRow.dType == DataType.WaveformMin || vRow.dType == DataType.WaveformMinPerWeight || vRow.dType == DataType.WaveformMax || vRow.dType == DataType.WaveformMaxPerWeight) {
        try {
          vRow.refValue = Double.parseDouble(vRow.refValues);
        } catch (Exception ex) {
    ...
    if (rVals.length == 2 && start == '[') { // We are an interval range
      if (refs.length != 1) {
      ...
      vRow.expected = vRow.refValues + " " + cite + vRow.refCites;
      return true;
    } else { // It's more than one value,intervals, this is where things get tricky..
  ...
  protected boolean validate(ValidationRow vRow)
  {
    ...
    } else if (!Double.isNaN(vRow.refValue)) { // What is the % difference between the value and the test value
  ...
  protected boolean getResult(ValidationRow vRow)
  {
    ...
    if (vRow.dType == DataType.MeanPerWeight || vRow.dType == DataType.WaveformMinPerWeight || vRow.dType == DataType.WaveformMaxPerWeight) { // Take off the weight unit from the header and unit we got from the spreadsheet
      String weightHeader = "PatientWeight(" + vRow.weightUnit + ")";
      vRow.weight = resultData.get(weightHeader);
      if (vRow.weight == null) { // Maybe the result is in a different unit than our reference value
        String newUnit = null;
        String[] split;
      ...
      for (String s : assessments.keySet()) {
        if (s.indexOf(vRow.resultFile) > -1) {
          try {
            ...
            } else if (r.getClass().isEnum()) { // TODO will need to get the enum value at some point
  ...
  protected List<Double> getResults(String name, String unit)
  {
    ...
    if (results == null) { // Maybe the result is in a different unit than our reference value
  ...
  protected void CrossCheckValidationWithSchema(List<String> properties, List<ValidationRow> eData, String sheetName)
  {
    ...
    for (Method m : c.getMethods()) {
      if (m.getName().startsWith("get") && ScalarData.class.isAssignableFrom(m.getReturnType())) {
        ...
        if (!found) { // Missing a row for this property
    ...
    for (String property : properties) {
      if (!property.contains("-") && !cdm.contains(property)) {
        try { // Maybe this is a non Scalar method...
          c.getMethod("get" + property);
        } catch (Exception ex) {
          try { // Or a boolean method...
