...
TEST_F(TEST_FIXTURE_NAME, Result_Construction)
{
  ...
  auto doubleResult_value = BIOGEARS_NAMESPACE Result<double,int>::make(double_value);
  auto doubleResult_null = BIOGEARS_NAMESPACE Result<double,int>::make_error();

  auto structResult_value = BIOGEARS_NAMESPACE Result<range,int>::make(range{ 5.,5. });
  auto structResult_null = BIOGEARS_NAMESPACE Result<range,int>::make_error();

  auto pointerResult_value = BIOGEARS_NAMESPACE Result<range*,int>::make(&struct_value);
  auto pointerResult_null = BIOGEARS_NAMESPACE Result<range*,int>::make_error();
...
TEST_F(TEST_FIXTURE_NAME, Result_unwrap_and_dump)
{
  ...
  double double_value = 9.9;
  auto doubleResult_value = BIOGEARS_NAMESPACE Result<double, int>::make(double_value);
  auto doubleResult_null = BIOGEARS_NAMESPACE Result<double, int>::make_error(5);
...
TEST_F(TEST_FIXTURE_NAME, Result_match)
{
  ...
  double double_value = 9.9;
  {
    const auto doubleResult_value = BIOGEARS_NAMESPACE Result<double, int>::make(double_value);
    const auto doubleResult_null = BIOGEARS_NAMESPACE Result<double, int>::make_error();
...
TEST_F(TEST_FIXTURE_NAME, Result_map)
{
  ...
  double double_value = 9.9;
  {
    ...
    EXPECT_TRUE(doubleResult_value);
    auto map_result = doubleResult_value.map<bool>( [](const double&) { return BIOGEARS_NAMESPACE Result<bool,int>::make(true); } );
    EXPECT_EQ(map_result.unwrap(), true);
    EXPECT_TRUE(doubleResult_value);

    EXPECT_FALSE(doubleResult_null);
    map_result = doubleResult_null.map<bool>( [](const double&) {return  BIOGEARS_NAMESPACE Result<bool,int>::make(false); } );
...

  {
    ...
    EXPECT_TRUE(doubleResult_value);
    auto map_result = doubleResult_value.map<bool>([](double&&) { return BIOGEARS_NAMESPACE Result<bool, int>::make(true); });
    EXPECT_EQ(map_result.unwrap(), true);
    EXPECT_FALSE(doubleResult_value);

    EXPECT_FALSE(doubleResult_null);
    map_result = doubleResult_null.map<bool>([](double&&) {return  BIOGEARS_NAMESPACE Result<bool, int>::make(false); });
    EXPECT_TRUE(map_result.is_err());
    EXPECT_FALSE(doubleResult_null);
...
TEST_F(TEST_FIXTURE_NAME, Result_and_then)
{
  ...
  double double_value = 9.9;
  {
    ...
    auto and_then_result = doubleResult_value.and_then<bool>(
      [](double&&)  { return BIOGEARS_NAMESPACE Result<bool, int>::make(true); }
      );
    ...
    EXPECT_FALSE(doubleResult_null);
      ...
      [](double&&) { return BIOGEARS_NAMESPACE Result<bool, int>::make_error(true); }
    );
    ...
    EXPECT_TRUE(doubleResult_value);
    auto mand_result = doubleResult_value.mand<bool>( BIOGEARS_NAMESPACE Result<bool, int>::make(true) );
    EXPECT_TRUE(mand_result.is_ok());
    EXPECT_TRUE(doubleResult_value);

    EXPECT_FALSE(doubleResult_null);
		mand_result = doubleResult_null.mand<bool>(  BIOGEARS_NAMESPACE Result<bool, int>::make_error(5) );
    ...
    EXPECT_FALSE(doubleResult_null);
		...
		EXPECT_TRUE(doubleResult_value);
		auto mor_result = doubleResult_value.mor(BIOGEARS_NAMESPACE Result<double, int>::make(0));
		EXPECT_TRUE(mor_result.is_ok());
		EXPECT_FALSE(doubleResult_value);

		EXPECT_FALSE(doubleResult_null);
		mor_result = doubleResult_null.mor(BIOGEARS_NAMESPACE Result<double, int>::make_error(5));
		...
		[](int&&) { return BIOGEARS_NAMESPACE Result<double, bool>::make_error(false);}
		,false
		);
  ...
  const auto doubleResult_value = BIOGEARS_NAMESPACE Result<double, int>::make(double_value);
  const auto doubleResult_null = BIOGEARS_NAMESPACE Result<double, int>::make_error(500);

  auto result = doubleResult_null.and_then<std::string>
    ([](const double&) 
    { return BIOGEARS_NAMESPACE Result<std::string, int>::make("Divine Protector"); }
    );
