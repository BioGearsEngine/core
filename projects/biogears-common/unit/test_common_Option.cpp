...
TEST_F(TEST_FIXTURE_NAME, Option_Construction)
{
  ...
  auto doubleOption_value = BIOGEARS_NAMESPACE Option<double>(double_value);
  auto doubleOption_null = BIOGEARS_NAMESPACE Option<double>();

  auto structOption_value = BIOGEARS_NAMESPACE Option<range>(range{5.,5.});
  auto structOption_null = BIOGEARS_NAMESPACE Option<range>();

  auto pointerOption_value = BIOGEARS_NAMESPACE Option<range*>(&struct_value);
  auto pointerOption_null = BIOGEARS_NAMESPACE Option<range*>();

  auto someOption_value = BIOGEARS_NAMESPACE Option<double>::make(1.);
...
TEST_F(TEST_FIXTURE_NAME, Option_unwrap_and_dump)
{
...

  {
    ...
    auto option = BIOGEARS_NAMESPACE Option<range>(value);
...
TEST_F(TEST_FIXTURE_NAME, Option_match)
{
  ...
  {
    range or_range = { 1., {0.,0.,0.,0.} };
    const auto option = BIOGEARS_NAMESPACE Option<range>(value);
    const auto none = BIOGEARS_NAMESPACE Option<range>();
...

  {
    range or_range = { 1., {0.,0.,0.,0.} };
    auto option = BIOGEARS_NAMESPACE Option<range>(value);
    auto none = BIOGEARS_NAMESPACE Option<range>();
...
TEST_F(TEST_FIXTURE_NAME, Option_and_then)
{
  ...
  {
    range or_range = { 1., {0.,0.,0.,0.} };
    const auto const_option = BIOGEARS_NAMESPACE Option<range>(value);
    auto option = BIOGEARS_NAMESPACE Option<range>(value);

    auto r1 = const_option.and_then<double> ( [](const range& val) { return BIOGEARS_NAMESPACE Option<double>( val.first); } );
    EXPECT_TRUE(r1);
    EXPECT_TRUE(const_option);

    auto r2 = option.and_then<double>([](range&& val) { return BIOGEARS_NAMESPACE Option<double>(val.first); });
    EXPECT_TRUE(r2);
    EXPECT_FALSE(option);

    auto r3 = option.and_then<double>([](range&& val) { return BIOGEARS_NAMESPACE Option<double>(val.first); });
    EXPECT_FALSE(r3);
    EXPECT_FALSE(option);

    auto r4 = const_option.mand<double>( BIOGEARS_NAMESPACE Option<double>(5.00) );
    EXPECT_TRUE(r4);
    EXPECT_TRUE(const_option);

    auto r5 = option.mand<double>(BIOGEARS_NAMESPACE Option<double>(105.00));
    EXPECT_FALSE(r5);
    EXPECT_FALSE(option);

    auto r6 = option.mor( BIOGEARS_NAMESPACE Option<range>( 95.0, std::vector<double>{} ) );
    EXPECT_TRUE(r6);
    EXPECT_FALSE(option);
    EXPECT_EQ(95.00, r6.unwrap().first);

    auto r7 = option.or_else( [](void){ return BIOGEARS_NAMESPACE Option<range>();  } );
    EXPECT_FALSE(r7);
    EXPECT_FALSE(option);
